 ...
 * Faellen "You" -- dafuer sorgt die Sitzung, nicht die Darstellung.
 */
let names: readonly string[] = []
/** Der eigene Platz -- gegen Bots 0, online der zugeteilte. */
let mySeat = 0
const nameOf = (seat: number): string => names[seat] ?? `Seat ${seat}`
/** Verbform passend zum Namen: "You win", aber "Ben wins". */
const verbFor = (seat: number, you: string, other: string): string => (seat === mySeat ? you : other)
...
export function renderTable(current: Session, handlers: TableHandlers): void {
  ...
  $('roundInfo').textContent = `Round ${view.roundNumber} of ${view.totalRounds}`
        ...
        led.suit !== null
          ...
          : led.settled
            ...
            : view.currentTrick.length > 0
              ? 'no suit led yet'
              : 'new trick'
      const wer = view.turn === view.you ? 'your turn' : `${nameOf(view.turn)}'s turn`
